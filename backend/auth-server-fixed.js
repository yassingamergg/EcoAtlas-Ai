...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const result = stmt.run(username, email, passwordHash, realName, verificationCode, verificationExpires.toISOString());
...
app.post('/api/auth/verify', (req, res) => {
  try {
    ...
    const stmt = db.prepare('UPDATE users SET is_verified = 1, verification_code = NULL, verification_expires = NULL WHERE id = ?');
    stmt.run(user.id);
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!isValidPassword) {
      ...
      const updatedUser = db.prepare('SELECT failed_login_attempts FROM users WHERE id = ?').get(user.id);
...
app.post('/api/auth/forgot-password', async (req, res) => {
  try {
    ...
    const user = db.prepare('SELECT id, username, email, real_name FROM users WHERE email = ?').get(email);
    ...
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?)
    `);
    stmt.run(user.id, resetCode, resetExpires.toISOString());
...
app.post('/api/auth/reset-password', async (req, res) => {
  try {
    ...
    stmt.run(passwordHash, resetRecord.user_id);
